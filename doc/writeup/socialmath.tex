...
\subsection*{Abstract}
Mathematical proofs (nodes) and the dependencies among them (edges) have the structure of a directed acyclic graph (DAG). However, theorems and proofs are typically presented in a linear fashion, which fails to connotate the theorem's context. Therefore, we propose a web service to better present mathematical proof trees as DAGs. Supposedly, the visualization should  help users both navigate through the existing mathematical library and also easily contribute new proofs to expand the reach of the proof trees. Specification of proofs could either be very formal, in which case a program could verify them, or more relaxed and reviewed by other users in a social effort to find mathematical truth.

\keywords{proof trees, visualization, social network} % NOT required for Proceedings
...
The same idea can apply to other field of study as well. 
